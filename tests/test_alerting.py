...
class TestAlertRule:
    def test_gt_triggered(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="gt", cooldown_seconds=0)
    ...
    def test_lt_triggered(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="lt", cooldown_seconds=0)
        assert rule.evaluate(5.0) is True

    def test_gte_triggered(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="gte", cooldown_seconds=0)
        assert rule.evaluate(10.0) is True

    def test_lte_triggered(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="lte", cooldown_seconds=0)
        assert rule.evaluate(10.0) is True

    def test_eq_triggered(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="eq", cooldown_seconds=0)
        assert rule.evaluate(10.0) is True

    def test_cooldown(self):
        rule = AlertRule(name="test", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0, comparison="gt", cooldown_seconds=100.0)
        assert rule.evaluate(15.0) is True
...
class TestAlertManager:
    def test_add_remove_rule(self):
        mgr = AlertManager()
        rule = AlertRule(name="r1", description="", severity=AlertSeverity.WARNING,
                         metric_name="m", threshold=10.0)
        mgr.add_rule(rule)
    ...
    def test_evaluate_all_no_provider(self):
        mgr = AlertManager()
        mgr.add_rule(AlertRule(name="r1", description="", severity=AlertSeverity.WARNING,
                               metric_name="m", threshold=10.0))
    ...
    def test_evaluate_all_triggered(self):
        mgr = AlertManager()
        mgr.add_rule(AlertRule(name="r1", description="test alert", severity=AlertSeverity.WARNING,
                               metric_name="m", threshold=10.0, comparison="gt", cooldown_seconds=0))
    ...
    def test_listener_called(self):
        mgr = AlertManager()
        mgr.add_rule(AlertRule(name="r1", description="test", severity=AlertSeverity.WARNING,
                               metric_name="m", threshold=10.0, comparison="gt", cooldown_seconds=0))
        mgr.register_metric_provider("m", lambda: 15.0)
        received = []

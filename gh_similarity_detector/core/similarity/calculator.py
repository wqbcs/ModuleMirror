...
class SimilarityCalculator:
    ...
    def _find_similar_modules(
        ...
        candidate_counts = self.inverted_index.get_candidates(
            source_fp.winnowing_fingerprints
        )
        ...
        for candidate_id, overlap in candidate_counts.items():
            ...
            if self._cache_db:
                try:
                    cached = self._cache_db.get_similarity_cache(
                        source_module.id, candidate_id
                    )
                    if cached:
            ...
            ast_similarity = self._calculate_ast_similarity(
                source_fp.ast_fingerprints,
                candidate_fp.ast_fingerprints
            )
                    ...
                    try:
                        ...
                        if not verify_result.verified:
                            ...
                            if matched_snippet:
                                ...
                                matched_snippet["ast_node_sim"] = round(verify_result.node_similarity, 2)
                                matched_snippet["ast_struct_sim"] = round(verify_result.structure_similarity, 2)
                        else:
                            if matched_snippet:

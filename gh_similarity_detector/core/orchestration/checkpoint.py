...
class Checkpoint:
    ...
    def mark_failed(self, candidate_source: str, error: str) -> None:
        self.data['failed_candidates'].append({
            'source': candidate_source, 'error': error
        })
    ...
    def add_result(self, source_project: str, target_project: str,
                   match_count: int, statistics: Dict) -> None:
    ...
    def get_pending_candidates(self) -> List[str]:
        ...
        return [
            cs for cs in self.candidate_sources
            if cs not in completed and cs not in failed_sources
        ]

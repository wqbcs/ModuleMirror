...
class ProjectFetcher:
    ...
    def __init__(self, config: DetectionConfig):
        ...
        self.file_filter = FileFilter(
            ...
            exclude_patterns=config.file_filter.DEFAULT_EXCLUDE_PATTERNS if hasattr(config, 'file_filter') else config.exclude_file_patterns,
    ...
    def _is_gitlab_url(url: str) -> bool:
        return bool(re.match(r'https?://gitlab\.com/[^/]+/[^/]+', url) or
                     re.match(r'https?://gitlab\.[^/]+/[^/]+/[^/]+', url))
    ...
    def _fetch_local_project(self, path: str) -> Project:
        ...
        return Project(
            name=project_name, source=path, files=files,
            local_path=path, file_count=len(files)
        )
    ...
    def _fetch_via_api(
        ...
        repo_info = loop.run_until_complete(
            self.github_client.get_repo_info(owner, repo)
        )
        if not repo_info:
            return None
        ...
        tree = loop.run_until_complete(
            self.github_client.get_tree(owner, repo, default_branch)
        )
        if not tree:
            return None
        ...
        code_files_in_tree = [
            ...
            and self.file_filter.should_include(item.get('path', ''))
        ]
        ...
        return Project(
            name=project_name, source=url, files=files,
            url=url, file_count=len(files)
        )
    ...
    def _fetch_via_clone(self, url: str, project_name: str) -> Optional[Project]:
        ...
        return Project(
            name=project_name, source=url, files=files,
            url=url, local_path=temp_dir, file_count=len(files)
        )
    ...
    def _scan_directory(self, root_path: str) -> List[CodeFile]:
        ...
        for dirpath, dirnames, filenames in os.walk(root_path):
            dirnames[:] = [
                d for d in dirnames 
                if d not in self.file_filter.exclude_dirs
            ]
                ...
                try:
                    ...
                    if language:
                        files.append(CodeFile(
                            path=relative_path, content=content, language=language
                        ))

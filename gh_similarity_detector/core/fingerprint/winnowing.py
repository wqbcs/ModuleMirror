...
class Winnowing:
    ...
    def generate_fingerprints_from_code(
        self, code: str, language: str = "python", module_id: str = ""
    ) -> FingerprintSet:
        """直接从代码字符串生成指纹
        ...
        if len(tokens) < self.kgram_size:
            if tokens:
                fp = self.hasher.hash_sequence(tokens)
                return FingerprintSet(
                    module_id=module_id, winnowing_fingerprints={fp}, token_count=len(tokens)
                )
        ...
        return FingerprintSet(
            module_id=module_id, winnowing_fingerprints=fingerprints, token_count=len(tokens)
        )
...
def compute_continuity_score(
    ...
    for i in range(1, len(sorted_hashes)):
        ...
        pos2_diff = abs(positions2.get(curr_hash, 0) - positions2.get(prev_hash, 0))

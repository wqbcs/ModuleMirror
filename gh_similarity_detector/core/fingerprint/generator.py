...
class ASTFingerprintGenerator:
    ...
    def generate_ast_fingerprints(self, module: Module) -> Set[int]:
        ...
        try:
            ...
            logger.error(f"生成 AST 指纹失败 (module={module.id}, lang={module.language}): {e}", exc_info=True)
            return set()
...
class FingerprintGenerator:
    ...
    def generate_fingerprints_batch(
        ...
        if total <= 1 or self.config.parallelism <= 1:
            ...
            ExecutorClass = ProcessPoolExecutor if self.config.use_process_pool else ThreadPoolExecutor

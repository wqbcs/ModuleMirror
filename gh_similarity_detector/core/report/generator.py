...
class ReportSanitizer:
    SENSITIVE_PATTERNS = [
        ...
        (
            r"-----BEGIN (?:RSA |DSA |EC )?PRIVATE KEY-----[\s\S]+?-----END",
...
class ReportGenerator:
    ...
    def _generate_markdown_report(self, results: List[DetectionResult]) -> str:
        ...
        lines.extend(["---", "", "**免责声明**: 本报告仅供参考，不作为法律依据。", ""])

        return "\n".join(lines)

    def _format_result_markdown(self, result: DetectionResult, index: int) -> List[str]:
        lines = [
            ...
            for match in result.matches:
                ...
                if match.matched_code_snippet:
                    ...
                    max_lines = (
                        ...
                        if hasattr(self.config, "max_diff_lines")
                        else 200
                    )
                ...
                all_matches_data.append(
                    {
                        ...
                        "ast_similarity": round(match.ast_similarity, 2)
                        if match.ast_similarity
                        else None,

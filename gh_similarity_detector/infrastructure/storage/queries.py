...
class Queries:
        ...
        with self._get_conn() as conn:
            ...
            for file_path, file_modules in modules.items():
                for module in file_modules:
                    module_rows.append((
                        module.id, project.id, module.name, module.file_path,
                        module.module_type.value, module.source_code,
                        module.start_line, module.end_line, module.token_count,
                        module.language
                    ))
            if module_rows:
                conn.executemany('''
        ...
        with self._get_conn() as conn:
            ...
            if row:
                return {
                    ...
                    'file_path': row[3], 'module_type': row[4],
                    'start_line': row[5], 'end_line': row[6],
                    'token_count': row[7], 'language': row[8]
                }
            return None
        ...
        with self._get_conn() as conn:
            ...
            if row:
                return {
                    ...
                    'language': row[3], 'file_count': row[4],
                    'module_count': row[5], 'created_at': row[6],
                    'updated_at': row[7]
                }
            return None

    def get_module_fingerprints(
        self,
        module_id: str,
        fp_type: str = 'winnowing'
    ) -> Set[int]:
    ...
    def list_projects(self) -> List[Dict]:
        with self._get_conn() as conn:
            ...
            return [
                {
                    'id': row[0], 'name': row[1], 'url': row[2],
                    'language': row[3], 'module_count': row[4],
                    'updated_at': row[5]
                }
    ...
    def put_similarity_cache(
        ...
        with self._get_conn() as conn:
            ...
            ''', (
                source_module_id, target_module_id, similarity,
                winnowing_overlap, ast_similarity, datetime.now().isoformat()
            ))
    ...
    def batch_put_similarity_cache(self, entries: List[Dict]) -> None:
        ...
        with self._get_conn() as conn:
            ...
            ''', [
                (
                    e['source_module_id'], e['target_module_id'],
                    e['similarity'], e.get('winnowing_overlap'),
                    e.get('ast_similarity'), now
                )
                for e in entries
            ])
    ...
    def create_task(
        ...
        with self._get_conn() as conn:
            conn.execute('''
                ...
                VALUES (?, ?, ?, 'pending', 0.0, ?, ?)
            ''', (
                task_id, target_project, candidates,
    ...
    def get_task(self, task_id: str) -> Optional[Dict]:
        with self._get_conn() as conn:
            ...
            if row:
                return {
                    'id': row[0], 'target_project': row[1], 'candidates': row[2],
                    'status': row[3], 'progress': row[4], 'result_path': row[5],
    ...
    def list_tasks(self, status: Optional[str] = None) -> List[Dict]:
        with self._get_conn() as conn:
            ...
            return [
                {'id': r[0], 'target_project': r[1], 'status': r[2], 'progress': r[3], 'created_at': r[4]}
    ...
    def export_to_json(self, output_path: str) -> int:
        with self._get_conn() as conn:
            projects = conn.execute('SELECT id, name, url, language, created_at, updated_at FROM projects').fetchall()
            export_data = {
            ...
            for proj in projects:
                proj_data = {
                    'id': proj[0], 'name': proj[1], 'url': proj[2],
                    'language': proj[3], 'created_at': proj[4], 'updated_at': proj[5],
                    'modules': []
                }
                modules = conn.execute('SELECT id, name, file_path, module_type, language FROM modules WHERE project_id = ?', (proj[0],)).fetchall()
                for mod in modules:
                    mod_data = {'id': mod[0], 'name': mod[1], 'file_path': mod[2], 'module_type': mod[3], 'language': mod[4], 'fingerprints': []}
                    fps = conn.execute('SELECT fingerprint, fingerprint_type FROM fingerprints WHERE module_id = ?', (mod[0],)).fetchall()
                    for fp in fps:
    ...
    def import_from_json(self, input_path: str) -> int:
        ...
        with self._get_conn() as conn:
            for proj_data in data.get('projects', []):
                existing = conn.execute('SELECT 1 FROM projects WHERE id = ?', (proj_data['id'],)).fetchone()
                if existing:
                    continue
                conn.execute(
                    'INSERT INTO projects (id, name, url, language, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
                    (proj_data['id'], proj_data['name'], proj_data.get('url', ''),
                     proj_data.get('language', ''), proj_data.get('created_at', ''), proj_data.get('updated_at', ''))
                )
                for mod_data in proj_data.get('modules', []):
                    conn.execute(
                        'INSERT INTO modules (id, name, file_path, module_type, language, project_id) VALUES (?, ?, ?, ?, ?, ?)',
                        (mod_data['id'], mod_data['name'], mod_data.get('file_path', ''),
                         mod_data.get('module_type', 'function'), mod_data.get('language', ''), proj_data['id'])
                    )
    ...
    def get_detection_history(
        ...
        with self._get_conn() as conn:
            ...
            return [
                {
                    "id": r[0], "target_project": r[1], "candidate_count": r[2],
                    "match_count": r[3], "avg_similarity": r[4], "max_similarity": r[5],
    ...
    def get_detection_trend(
        ...
        with self._get_conn() as conn:
            ...
            return [
                {
                    "created_at": r[0], "match_count": r[1],
                    "avg_similarity": r[2], "max_similarity": r[3],
                }
                for r in rows
            ]
